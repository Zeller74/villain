...
const DEFAULT_CHARACTER_ID = CHARACTERS_DATA[0].id;
function newRoomId() {
...
function emitRoomState(io, roomId) {
    ...
    const publicPlayers = room.players.map((p) => {
        ...
        const board = p.board ?? { moverAt: 0, locations: [] };
        const locations = (board.locations ?? []).map((loc, idx) => ({
            id: loc?.id ?? `L${idx + 1}`,
...
function emitPrivateStates(io, roomId) {
    const room = rooms.get(roomId);
    if (!room)
        return;
...
function leaveCurrentRoom(socket, io, opts) {
    ...
    const sys = { id: (0, nanoid_1.nanoid)(8), ts: Date.now(), playerId: "system", name: "System", text };
...
function makeLocation(ix, label) {
    return {
...
function makeEmptyBoard() {
    return {
        moverAt: 0,
        locations: [0, 1, 2, 3].map(i => makeLocation(i)),
    };
}
...
function reshuffleFromDiscardIntoDeck(p) {
    if (p.zones.deck.length > 0)
        return false;
...
function buildLogItem(room, e) {
    ...
    if (e.type === "play_effect" && e.data.type === "play_effect") {
        ...
        for (const p of room.players) {
            ...
            const onBoard = p.board.locations.some(loc => loc.bottom.some(c => c.id === cardId) || loc.top.some(c => c.id === cardId));
            if (inDiscard) {
                label = inDiscard.label;
                break;
            }
            if (inHand) {
                label = inHand.label;
...
function emitRoomLog(io, roomId) {
    const room = rooms.get(roomId);
    if (!room)
        return;
    // Broadcast a sanitized view (most recent first)
    const items = room.log.slice(-25).map(e => buildLogItem(room, e)).reverse();
    io.to(roomId).emit("room:log", { items });
}
function pushLog(io, roomId, entry) {
    const room = rooms.get(roomId);
    if (!room)
        return;
    room.log.push(entry);
    if (room.log.length > 25)
        room.log.splice(0, room.log.length - 25);
    emitRoomLog(io, roomId);
}
function shuffleDiscardIntoDeck(p) {
...
function shuffleFateDiscardIntoDeck(p) {
...
function advanceToNextActive(room) {
...
function makeBoardFrom(locTpls) {
    const get = (i) => locTpls[i] ?? { name: `Location ${i + 1}`, actions: [], topSlots: 0 };
    const mk = (i, id) => {
        const t = get(i);
        return {
            id,
...
function isCharacterId(x) {
    return !!x && (x in CHARACTERS);
}
function getCharacter(id) {
    const key = isCharacterId(id) ? id : DEFAULT_CHARACTER_ID;
    return CHARACTERS[key];
}
function nowId() { return (0, nanoid_1.nanoid)(8); }
function placeFromFateTop(player, matcher, locIndex = 0) {
    ...
    if (!card) {
        ...
        if (idx >= 0)
            card = player.zones.fateDiscard.splice(idx, 1)[0];
    }
...
function applyCharacterSetup(player) {
...
function isHero(c) { return c.type === "Hero"; }
io.on("connection", (socket) => {
    ...
    socket.on("room:create", (payload, ack) => {
        ...
        const room = { id: roomId, ownerId: socket.id, players: [], game: { phase: "lobby", turn: 1, activePlayerId: null }, messages: [], log: [] };
        rooms.set(roomId, room);
        //join as player
        const player = { id: socket.id, name, ready: false, characterId: DEFAULT_CHARACTER_ID, zones: { deck: [], hand: [], discard: [], fateDeck: [], fateDiscard: [] }, board: makeEmptyBoard(), power: 0, trust: 0, handPublic: false, };
    ...
    socket.on("room:join", (payload, ack) => {
        ...
        if (!room.players.some(p => p.id === socket.id)) {
            ...
            if (!room.game.activePlayerId && room.players.length > 0) {
                const first = room.players[0];
                if (first)
                    room.game.activePlayerId = first.id;
            }
        }
    ...
    socket.on("lobby:chooseCharacter", (payload, ack) => {
        const roomId = socket.data.roomId;
        if (!roomId)
            return ack?.({ ok: false, error: "not in a room" });
        const room = rooms.get(roomId);
        ...
        if (room.game.phase !== "lobby")
            return ack?.({ ok: false, error: "not in lobby" });
        const id = (payload?.characterId || "").trim();
        if (!id || !(id in CHARACTERS))
            return ack?.({ ok: false, error: "invalid character id" });
        ...
        const sys = { id: (0, nanoid_1.nanoid)(8), ts: Date.now(), playerId: "system", name: "System", text: `${me.name} chose ${id}` };
        room.messages.push(sys);
        room.messages = room.messages.slice(-100);
        io.to(roomId).emit("chat:msg", { roomId, msg: sys });
    });
    ...
    socket.on("lobby:start", (ack) => {
        ...
        if (!isOwner(socket, room))
            return ack?.({ ok: false, error: "owner only" });
        if (room.players.length < 2)
            return ack?.({ ok: false, error: "need at least 2 players" });
        if (!allReady(room))
            return ack?.({ ok: false, error: "not all ready" });
        for (const p of room.players) {
        ...
        emitRoomLog(io, room.id);
        //\system message
        const sys = { id: (0, nanoid_1.nanoid)(8), ts: Date.now(), playerId: "system", name: "System", text: "Game started!" };
        room.messages.push(sys);
        room.messages = room.messages.slice(-100);
        io.to(roomId).emit("chat:msg", { roomId, msg: sys });
    });
    ...
    socket.on("game:draw", (payload, ack) => {
        ...
        if (room.game.phase !== "playing")
            return ack?.({ ok: false, error: "game not started" });
        if (room.game.activePlayerId !== socket.id) {
            return ack?.({ ok: false, error: "not your turn" });
        }
        const me = room.players.find(p => p.id === socket.id);
        ...
        const drawnIds = []; // collect while drawing
        for (let i = 0; i < n; i++) {
    ...
    socket.on("game:playToLocation", (payload, ack) => {
        const roomId = socket.data.roomId;
        if (!roomId)
            return ack?.({ ok: false, error: "not in a room" });
        const room = rooms.get(roomId);
        ...
        if (idx === -1)
            return ack?.({ ok: false, error: "card not in hand" });
        ...
        if (!loc)
            return ack?.({ ok: false, error: "bad location" });
        if (loc.locked)
            return ack?.({ ok: false, error: "location is locked" });
        ...
        pushLog(io, roomId, {
            ...
            actorId: socket.id,
            type: "play",
            data: { type: "play", cardId: card.id, locationIndex: k },
        });
    });
    socket.on("game:discard", (payload, ack) => {
        ...
        if (ids.length === 0)
            return ack?.({ ok: false, error: "no cards specified" });
        let count = 0;
        for (const id of ids) {
        ...
        if (count === 0)
            return ack?.({ ok: false, error: "card(s) not in hand" });
    ...
    socket.on("pile:getDiscard", (payload, ack) => {
        const roomId = socket.data.roomId;
        if (!roomId)
            return ack?.({ ok: false, error: "not in a room" });
        const room = rooms.get(roomId);
    ...
    socket.on("game:moveCard", (payload, ack) => {
        const roomId = socket.data.roomId;
        if (!roomId)
            return ack?.({ ok: false, error: "not in a room" });
        const room = rooms.get(roomId);
        ...
        if (!fromLoc || !toLoc)
            return ack?.({ ok: false, error: "bad locations" });
        const idx = fromLoc.bottom.findIndex(c => c.id === payload.cardId);
        if (idx === -1)
            return ack?.({ ok: false, error: "card not in source location (bottom)" });
        const srcCard = fromLoc.bottom[idx];
        if (srcCard?.locked)
            return ack?.({ ok: false, error: "card is locked" });
        if (toLoc.locked)
            return ack?.({ ok: false, error: "destination locked" });
        ...
        pushLog(io, roomId, {
            ...
            actorId: socket.id,
            type: "move",
            data: { type: "move", cardId: card.id, from: from, to: to, fromIndex: idx, toIndex },
        });
    });
    socket.on("log:undoSelf", (ack) => {
        const roomId = socket.data.roomId;
        if (!roomId)
            return ack?.({ ok: false, error: "not in a room" });
        const room = rooms.get(roomId);
        ...
        if (!last)
            return ack?.({ ok: false, error: "nothing to undo" });
        if (last.actorId !== socket.id)
            return ack?.({ ok: false, error: "only your last action can be undone" });
        if (last.undone)
            return ack?.({ ok: false, error: "already undone" });
        const me = room.players.find(p => p.id === socket.id);
        ...
        if (last.type === "draw" && last.data.type === "draw") {
            ...
            if (idx === -1)
                return ack?.({ ok: false, error: "card not on board anymore" });
            ...
            if (j === -1)
                return ack?.({ ok: false, error: "card not in destination anymore" });
            ...
            if (idx === -1)
                return ack?.({ ok: false, error: "cannot undo: card moved from hand" });
            const card = me.zones.hand.splice(idx, 1)[0];
            ...
            if (d.target === "location") {
                ...
                if (j === -1)
                    return ack?.({ ok: false, error: "card not found" });
            ...
            if (j === -1)
                return ack?.({ ok: false, error: "card not in discard" });
            ...
            const target = room.players.find(p => p.id === d.targetId);
            if (!target)
                return ack?.({ ok: false, error: "target not found for undo" });
            // locate the card in fateDeck
            const j = target.zones.fateDeck.findIndex(c => c.id === d.cardId);
            if (j === -1)
                return ack?.({ ok: false, error: "card not in fate deck" });
            const card = target.zones.fateDeck.splice(j, 1)[0];
            if (!card)
                return ack?.({ ok: false, error: "deck splice failed" });
    ...
    socket.on("game:removeCard", (payload, ack) => {
        const roomId = socket.data.roomId;
        if (!roomId)
            return ack?.({ ok: false, error: "not in a room" });
        const room = rooms.get(roomId);
        ...
        const idx = fromLoc.bottom.findIndex(c => c.id === payload.cardId);
        if (idx === -1)
            return ack?.({ ok: false, error: "card not on that location (bottom)" });
        ...
        pushLog(io, roomId, {
            ...
            actorId: socket.id,
            type: "remove",
            data: { type: "remove", cardId: card.id, from: from, fromIndex: idx },
        });
    });
    socket.on("game:reshuffleDeck", (ack) => {
        const roomId = socket.data.roomId;
        if (!roomId)
            return ack?.({ ok: false, error: "not in a room" });
        const room = rooms.get(roomId);
        ...
        if (moved === 0)
            return ack?.({ ok: false, error: "discard is empty" });
        emitRoomState(io, roomId);
        pushLog(io, roomId, {
            id: (0, nanoid_1.nanoid)(8),
            ts: Date.now(),
    ...
    socket.on("pile:takeFromDiscard", (payload, ack) => {
        const roomId = socket.data.roomId;
        if (!roomId)
            return ack?.({ ok: false, error: "not in a room" });
        const room = rooms.get(roomId);
        ...
        if (!id)
            return ack?.({ ok: false, error: "missing cardId" });
        const fromIndex = me.zones.discard.findIndex(c => c.id === id);
        if (fromIndex === -1)
            return ack?.({ ok: false, error: "card not in your discard" });
    ...
    socket.on("power:change", (payload, ack) => {
        const roomId = socket.data.roomId;
        if (!roomId)
            return ack?.({ ok: false, error: "not in a room" });
        const room = rooms.get(roomId);
        ...
        if (!Number.isFinite(raw) || raw === 0)
            return ack?.({ ok: false, error: "no change" });
    ...
    socket.on("pawn:set", (payload, ack) => {
        const roomId = socket.data.roomId;
        if (!roomId)
            return ack?.({ ok: false, error: "not in a room" });
        const room = rooms.get(roomId);
        ...
        const to = raw;
        if (me.board.locations[to].locked)
            return ack?.({ ok: false, error: "location is locked" });
    ...
    socket.on("board:toggleLocationLock", (payload, ack) => {
        const roomId = socket.data.roomId;
        if (!roomId)
            return ack?.({ ok: false, error: "not in a room" });
        const room = rooms.get(roomId);
        ...
        emitRoomState(io, roomId);
        pushLog(io, roomId, {
            id: (0, nanoid_1.nanoid)(8), ts: Date.now(), actorId: socket.id, type: "lock",
            data: { type: "lock", target: "location", loc: locIdx, prev, next },
        });
        ack?.({ ok: true });
    });
    socket.on("board:toggleCardLock", (payload, ack) => {
        const roomId = socket.data.roomId;
        if (!roomId)
            return ack?.({ ok: false, error: "not in a room" });
        const room = rooms.get(roomId);
        ...
        if (locIdx === null || row === null)
            return ack?.({ ok: false, error: "card not on your board" });
    ...
    socket.on("card:deltaStrength", (payload, ack) => {
        const roomId = socket.data.roomId;
        if (!roomId)
            return ack?.({ ok: false, error: "not in a room" });
        const room = rooms.get(roomId);
    ...
    socket.on("fate:reshuffleDeck", (payload, ack) => {
        const roomId = socket.data.roomId;
        if (!roomId)
            return ack?.({ ok: false, error: "not in a room" });
        const room = rooms.get(roomId);
        ...
        if (moved === 0)
            return ack?.({ ok: false, error: "fate discard is empty" });
        emitRoomState(io, roomId);
        pushLog(io, roomId, {
            id: (0, nanoid_1.nanoid)(8),
            ts: Date.now(),
    ...
    socket.on("fate:start", (payload, ack) => {
        const roomId = socket.data.roomId;
        if (!roomId)
            return ack?.({ ok: false, error: "not in a room" });
        const room = rooms.get(roomId);
    ...
    socket.on("fate:choosePlay", (payload, ack) => {
        const roomId = socket.data.roomId;
        if (!roomId)
            return ack?.({ ok: false, error: "not in a room" });
        const room = rooms.get(roomId);
        ...
        if (!id)
            return ack?.({ ok: false, error: "missing cardId" });
        if (!sess.drawn.some(c => c.id === id))
            return ack?.({ ok: false, error: "card not in fate choices" });
        sess.chosenId = id;
        ack?.({ ok: true });
    });
    socket.on("fate:placeSelected", (payload, ack) => {
        const roomId = socket.data.roomId;
        if (!roomId)
            return ack?.({ ok: false, error: "not in a room" });
        const room = rooms.get(roomId);
        ...
        if (room.game.phase !== "playing")
            return ack?.({ ok: false, error: "game not started" });
        const sess = room.fate;
        if (!sess || sess.actorId !== socket.id)
            return ack?.({ ok: false, error: "no active fate" });
        const target = room.players.find(p => p.id === sess.targetId);
        if (!target) {
            delete room.fate;
            return ack?.({ ok: false, error: "target missing" });
        }
        ...
        const chosen = room.fate.chosenId
            ...
            : room.fate.drawn[0];
        if (!chosen)
            return ack?.({ ok: false, error: "no chosen card" });
        const other = room.fate.drawn.find(c => c && c.id !== chosen.id) || null;
        loc.top.push(chosen);
        if (other)
            target.zones.fateDiscard.push(other);
        delete room.fate;
        emitRoomState(io, roomId);
        const data = other
            ? { type: "fate_play", targetId: target.id, playedCardId: chosen.id, locationIndex: i, discardedCardId: other.id }
            : { type: "fate_play", targetId: target.id, playedCardId: chosen.id, locationIndex: i };
        pushLog(io, roomId, {
            id: (0, nanoid_1.nanoid)(8),
            ts: Date.now(),
            actorId: socket.id,
    ...
    socket.on("fate:getDiscard", (payload, ack) => {
        const roomId = socket.data.roomId;
        if (!roomId)
            return ack?.({ ok: false, error: "not in a room" });
        const room = rooms.get(roomId);
    ...
    socket.on("fate:startFromDiscard", (payload, ack) => {
        const roomId = socket.data.roomId;
        if (!roomId)
            return ack?.({ ok: false, error: "not in a room" });
        const room = rooms.get(roomId);
        ...
        if (idx === -1)
            return ack?.({ ok: false, error: "card not in fate discard" });
        const [card] = target.zones.fateDiscard.splice(idx, 1);
        if (!card)
            return ack?.({ ok: false, error: "card missing" });
    ...
    socket.on("fate:discardBoth", (_, ack) => {
        const roomId = socket.data.roomId;
        if (!roomId)
            return ack?.({ ok: false, error: "not in a room" });
        const room = rooms.get(roomId);
    ...
    socket.on("board:moveTop", (payload, ack) => {
        const roomId = socket.data.roomId;
        if (!roomId)
            return ack?.({ ok: false, error: "not in a room" });
        const room = rooms.get(roomId);
        ...
        if (j === -1)
            return ack?.({ ok: false, error: "card not in top at source" });
        ...
        pushLog(io, roomId, {
            ...
            actorId: socket.id,
            type: "move_top",
            data: { type: "move_top", cardId, from: from, to: to, fromIndex: j, toIndex: toLoc.top.length - 1 },
        });
        ack?.({ ok: true });
    });
    socket.on("board:discardTop", (payload, ack) => {
        const roomId = socket.data.roomId;
        if (!roomId)
            return ack?.({ ok: false, error: "not in a room" });
        const room = rooms.get(roomId);
        ...
        const cardId = (payload?.cardId || "").trim();
        if (!Number.isInteger(iRaw) || iRaw < 0 || iRaw > 3)
            return ack?.({ ok: false, error: "bad location index" });
        if (!cardId)
            return ack?.({ ok: false, error: "missing cardId" });
        const i = iRaw;
        ...
        if (j === -1)
            return ack?.({ ok: false, error: "card not in top at that location" });
    ...
    socket.on("meta:getCharacters", (_, ack) => {
    ...
    socket.on("game:playEffect", (payload, ack) => {
        const roomId = socket.data.roomId;
        if (!roomId)
            return ack?.({ ok: false, error: "not in a room" });
        const room = rooms.get(roomId);
        ...
        const cardId = (payload?.cardId ?? "").trim();
        if (!cardId)
            return ack?.({ ok: false, error: "no card specified" });
        ...
        if (!taken)
            return ack?.({ ok: false, error: "failed to remove card" });
        taken.faceUp = true;
    ...
    socket.on("fatePeek:start", (payload, ack) => {
        const roomId = socket.data.roomId;
        if (!roomId)
            return ack?.({ ok: false, error: "not in a room" });
        const room = rooms.get(roomId);
    ...
    socket.on("fatePeek:confirm", (payload, ack) => {
        const roomId = socket.data.roomId;
        if (!roomId)
            return ack?.({ ok: false, error: "not in a room" });
        const room = rooms.get(roomId);
        ...
        if (!sess || sess.actorId !== socket.id)
            return ack?.({ ok: false, error: "no active peek" });
        const target = room.players.find(p => p.id === sess.targetId);
        if (!target) {
            delete room.fatePeek;
            return ack?.({ ok: true });
        }
        const byId = new Map(sess.drawn.map(c => [c.id, c]));
    ...
    socket.on("fatePeek:cancel", (_, ack) => {
        const roomId = socket.data.roomId;
        if (!roomId)
            return ack?.({ ok: false, error: "not in a room" });
        const room = rooms.get(roomId);
    ...
    socket.on("fateDiscard:return", (payload, ack) => {
        const roomId = socket.data.roomId;
        if (!roomId)
            return ack?.({ ok: false, error: "not in a room" });
        const room = rooms.get(roomId);
        ...
        const cardId = (payload?.cardId || "").trim();
        if (!targetId || !cardId)
            return ack?.({ ok: false, error: "bad request" });
        ...
        const [card] = target.zones.fateDiscard.splice(idx, 1);
        if (!card)
            return ack?.({ ok: false, error: "splice failed" });
        target.zones.fateDeck.push(card);
    ...
    socket.on("char:toggleHandReveal", (_, ack) => {
        const roomId = socket.data.roomId;
        if (!roomId)
            return ack?.({ ok: false, error: "not in a room" });
        const room = rooms.get(roomId);
    ...
    socket.on("fateSift:start", (payload, ack) => {
        const roomId = socket.data.roomId;
        if (!roomId)
            return ack?.({ ok: false, error: "not in a room" });
        const room = rooms.get(roomId);
    ...
    socket.on("fateSift:choose", (payload, ack) => {
        ...
        if (!sess || sess.actorId !== socket.id)
            return ack?.({ ok: false, error: "no active sift" });
        const target = room.players.find(p => p.id === sess.targetId);
        if (!target) {
        ...
        if (other)
            target.zones.fateDeck.push(other);
    ...
    socket.on("fateSift:cancel", (_, ack) => {
        const roomId = socket.data.roomId;
        if (!roomId)
            return ack?.({ ok: false, error: "not in a room" });
        const room = rooms.get(roomId);
    ...
    socket.on("trust:change", (payload, ack) => {
        const room = rooms.get(socket.data.roomId);
        if (!room)
            return ack?.({ ok: false, error: "room not found" });
        if (room.game.phase !== "playing")
            return ack?.({ ok: false, error: "not playing" });
        if (room.game.activePlayerId !== socket.id)
            return ack?.({ ok: false, error: "not your turn" });
        const me = room.players.find(p => p.id === socket.id);
        if (!me)
    ...
    socket.on("lotso:flipBuzz", (_, ack) => {
        const roomId = socket.data.roomId;
        if (!roomId)
            return ack?.({ ok: false, error: "not in a room" });
        const room = rooms.get(roomId);
        ...
        if (!me)
            return ack?.({ ok: false, error: "player not found" });
        if (me.characterId !== "lotso")
            return ack?.({ ok: false, error: "only Lotso can flip Buzz" });
        ...
        for (let i = 0; i < 4; i++) {
            ...
            if (tIdx !== -1) {
                found = { loc: i, row: "top", idx: tIdx };
                break;
            }
            const bIdx = L.bottom.findIndex(c => c.label === "Demo Mode Buzz Lightyear");
            if (bIdx !== -1) {
                found = { loc: i, row: "bottom", idx: bIdx };
                break;
            }
        }
        if (!found)
            return ack?.({ ok: false, error: "Buzz (any side) not found on your board" });
        const L = me.board.locations[found.loc];
        if (!L)
            return ack?.({ ok: false, error: "location missing" });
        if (found.row === "top") {
            // Remove Buzz from TOP → add Demo Buzz to BOTTOM
            const old = L.top.splice(found.idx, 1)[0];
            if (!old)
                return ack?.({ ok: false, error: "Buzz index invalid (top)" });
            ...
            if (!old)
                return ack?.({ ok: false, error: "Buzz index invalid (bottom)" });
            const next = makeBuzz();
            L.top.push(next);
            pushLog(io, roomId, {
                id: nowId(),
    ...
    socket.on("fateFindHero:start", (payload, ack) => {
        const roomId = socket.data.roomId;
        if (!roomId)
            return ack?.({ ok: false, error: "not in a room" });
        const room = rooms.get(roomId);
        ...
        const heroExists = target.zones.fateDeck.some(isHero) ||
        ...
        while (safety-- > 0) {
            ...
            if (!c) {
                ...
                if (target.zones.fateDiscard.length === 0)
                    break;
    ...
    socket.on("fate:discardSelected", (_, ack) => {
        const roomId = socket.data.roomId;
        if (!roomId)
            return ack?.({ ok: false, error: "not in a room" });
        const room = rooms.get(roomId);
